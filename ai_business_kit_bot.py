...
# ---------------------------
# СИСТЕМНЫЕ ПРОМПТЫ ДЛЯ ИИ (ЕДИНАЯ СХЕМА)
# ---------------------------
def _env_or_default(key: str, default: str) -> str:
    return (os.getenv(key) or default).strip()

AI_SYSTEM_PROMPT_SETUP_RAW = _env_or_default(
    "AI_SYSTEM_PROMPT_SETUP",
...
AI_SYSTEM_PROMPT_UNIVERSAL_RAW = _env_or_default(
    "AI_SYSTEM_PROMPT_UNIVERSAL",
    ...
    "Формат: 1) Заголовок результата; 2) Готовый результат; 3) «Дальше» — 2–3 шага."
)

AI_SYSTEM_PROMPT_BRAND_RAW = _env_or_default(
    "AI_SYSTEM_PROMPT_BRAND",
    ...
    "«{PRODUCT_NAME}», чем он помогает. В конце дай 2–3 чётких следующих шага. Контакт: {BRAND_SUPPORT_TG}."
)

AI_SYSTEM_PROMPT_PAY_RAW = _env_or_default(
    "AI_SYSTEM_PROMPT_PAY",
...
AI_SYSTEM_PROMPT_USER_DEMO_RAW = _env_or_default(
    "AI_SYSTEM_PROMPT_USER_DEMO",
...
AI_SYSTEM_PROMPT_USER_RAW = _env_or_default(
...
AI_SYSTEM_PROMPT_ADMIN_RAW = _env_or_default(
...
def kb_after_payment(is_admin: bool = False) -> InlineKeyboardMarkup:
    """
    Меню после оплаты.
    Показывается пользователю, у которого активирован доступ.
    """
    kb = InlineKeyboardBuilder()

    # Основные действия после покупки
    kb.button(text="📦 Скачать материалы", callback_data="get_files_again")
    kb.button(text="🤖 ИИ-помощник",       callback_data="open_ai_modes")
    kb.button(text="ℹ️ О нас",             callback_data="open_about")
    kb.button(text="💬 Поддержка",         callback_data="support_request")
    ...
    if is_admin:
        ...
        kb.adjust(2, 2, 1)   # две строки по 2 + админская отдельная
    else:
        kb.adjust(2, 2)      # две строки по 2 кнопки
...
async def on_startup():
   ...
   # Запускаем heartbeat корректно (если включён в ENV)
    ...
    # --- Preview системных промптов в лог (безопасно) ---
    try:
        logging.info("[PROMPT_USER] %s", _fmt_prompt(AI_SYSTEM_PROMPT_USER_RAW)[:160].replace("\n", " "))
        logging.info("[PROMPT_ADMIN] %s", _fmt_prompt(AI_SYSTEM_PROMPT_ADMIN_RAW)[:160].replace("\n", " "))
    except Exception as e:
        logging.warning("Prompt preview skipped: %s", e)
