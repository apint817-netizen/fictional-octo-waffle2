...
def _verified_home_text() -> str:
    return (
        ...
        "Теперь у вас есть всё, чтобы запустить свой цифровой продукт с ИИ 💼\n\n"
...
async def approve_payment_handler(callback: types.CallbackQuery):
    ...
    даём «вау»-уведомление, выдаём материалы и переводим в экран купившего.
    ...
    try:
        ...
        await bot.send_message(ADMIN_ID, "⚠️ Некорректный формат callback-данных approve_*", parse_mode="HTML")
        return
    ...
    # «Вау»-уведомление пользователю (с акцентом, что можно использовать промпты прямо в боте)
    try:
        await bot.send_message(
            user_id,
            "✅ <b>Оплата успешно подтверждена!</b>\n\n"
            "Поздравляем 🎉 Вы получили доступ к <b>AI Business Kit</b> — готовому инструменту для запуска своего цифрового продукта с ИИ.\n\n"
            "Теперь вы можете:\n"
            "• Скачать все материалы, шаблоны и гайды\n"
            "• Использовать 100 готовых ChatGPT-промптов прямо внутри этого бота 🧠\n"
            "• Общаться с ИИ-помощником или использовать стандартный GPT-чат\n"
            "• Настроить и запустить собственного Telegram-бота под свой бизнес\n\n"
            "💡 Всё готово, даже если у вас нет отдельного ChatGPT-аккаунта — используйте встроенного GPT прямо здесь!\n\n"
            "🚀 Начните сегодня — и уже завтра у вас будет свой работающий цифровой продукт!",
            parse_mode="HTML"
        )
    except Exception as e:
        logging.warning("Welcome message send failed: %s", e)
    ...
    try:
        ...
        # 1) Отдельное уведомление админу
        await bot.send_message(
            ADMIN_ID,
