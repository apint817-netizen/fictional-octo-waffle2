...
def kb_start(is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    kb.button(text="🤖 ИИ: режимы", callback_data="ai_choice")
...
def kb_after_payment(is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    kb.button(text="🤖 ИИ: режимы", callback_data="ai_choice_main")                       # уже есть ai_open
...
def kb_verification_back() -> InlineKeyboardMarkup:
    ...
    kb.button(text="🤖 ИИ: режимы", callback_data="ai_choice")        # переименованная
    kb.button(text="💬 Поддержка", callback_data="support_request")
...
async def ai_universal_open(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not is_user_verified(uid):
        if DEMO_AI_ENABLED:
            ...
            await state.update_data(ai_is_admin=False, ai_mode="universal", ai_force_demo=True)
            text = (
                "🧪 Демо-режим включён. Доступно до "
                f"{DEMO_AI_DAILY_LIMIT} сообщений в день."
            )
            await safe_edit(
                callback.message,
                text=text if callback.message.text is not None else None,
            ...
            await safe_edit(
                callback.message,
                text=(
                    "⚠️ Полный доступ к этому режиму доступен после оплаты.\n"
                    "Нажмите ‘Оплата по СБП (QR)’ в меню."
                ),
                caption=None,
...
async def ai_brand_open_cb(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        (
            "ℹ️ <b>Режим: О продукте</b>\n"
            "Спроси: из чего состоит набор, для кого, чем полезен."
        ),
        reply_markup=kb_ai_chat(is_admin=False),
        parse_mode="HTML"
    )
